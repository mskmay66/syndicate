...
class Setup(Vertical):
    ...
    def compose(self) -> ComposeResult:
        with VerticalScroll(id="setup_scroll"):
            ...
            yield ApiKeyInput(id="broker_secret", provider="alpaca secret")
    ...
    def model_api_submitted(self, event: Input.Submitted) -> None:
        ...
        api_key = event.value
        self.callbacks.get("model_api_updated", lambda x: None)(api_key)
    ...
    def broker_api_submitted(self, event: Input.Submitted) -> None:
        ...
        api_key = event.value
        self.callbacks.get("broker_api_updated", lambda x: None)(api_key)

    @on(Input.Submitted, "#broker_secret_input")
    def broker_secret_submitted(self, event: Input.Submitted) -> None:
    ...
    def technical_api_submitted(self, event: Input.Submitted) -> None:
        ...
        api_key = event.value
        self.callbacks.get("technical_api_updated", lambda x: None)(api_key)
    ...
    def cron_selection_changed(self, event: Select.Changed) -> None:
        ...
        self.callbacks.get("cron_updated", lambda x: None)(cron_choice)

    @on(Input.Submitted, "#cron_expression_input")
    def cron_expression_submitted(self, event: Input.Submitted) -> None:
    ...
    @on(Select.Changed, "#guardrails_select")
    def guardrails_selection_changed(self, event: Select.Changed) -> None:
