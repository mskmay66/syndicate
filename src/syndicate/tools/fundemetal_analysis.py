...
def get_balance_sheet(tickers: List[str]) -> str:
    ...
    for ticker in tickers:
        try:
            stock = yf.Ticker(ticker)
            balance_sheets[ticker] = _convert_timestamp_cols(
                stock.balance_sheet
            ).to_dict()
        except Exception as e:
...
def get_income_statement(tickers: List[str]) -> str:
    ...
    for ticker in tickers:
        try:
            stock = yf.Ticker(ticker)
            income_statements[ticker] = _convert_timestamp_cols(
                stock.income_stmt
            ).to_dict()
        except Exception as e:
...
def get_cashflow(tickers: List[str]) -> str:
    ...
    for ticker in tickers:
        try:
            stock = yf.Ticker(ticker)
            cashflows[ticker] = _convert_timestamp_cols(stock.cashflow).to_dict()
        except Exception as e:
